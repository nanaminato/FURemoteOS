// Desktop taskbar, extracted from screens/widgets/taskbar.dart so the
// desktop shell owns all its composite layout pieces (ARCHITECTURE.md § 17).
//
// Pure UI composition: start button, search box, running-app shelf, tray
// icons, clock, and the connection indicator popover.  Start-menu open
// state is read reactively from the shell VM's overlay notifier so the
// start-button highlight follows the actual mounted overlay.
...
class DesktopTaskbar extends ConsumerWidget {
  ...
  Widget _buildStartButton(ThemePalette palette, bool isStartOpen) {
    return InkWell(
